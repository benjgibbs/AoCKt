...
fun expand(id: Int, rules: Map<Int, List<List<Any>>>): List<String> {
        ...
        for (list in expansion.drop(1)) {
            ...
            while (choiceResult.isNotEmpty()) {
                ...
                for (nextPart in list) {
                    nextResult.add(prefix + nextPart)
                }
            }
...
fun main() {
    ...
    for (line in lines){
        if (line.isEmpty()) {
            parsingRules = false
            possible.addAll(expand(0, rules))
    ...
    println("Part1: $count")
    println("8: ${expand(8, rules)}")
    println("11: ${expand(11, rules)}")

    cache.clear()
    rules[8] = listOf(listOf(42))
    rules[11] = listOf(listOf(42, 31))
    val possibles2 = expand(0, rules)
    println("Part2: ${toCheck.filter { it in possibles2 }.count()}")
}

private fun checkExample() {
    val example = mapOf(
    ...
    val exampleResult =expand(0, example)
